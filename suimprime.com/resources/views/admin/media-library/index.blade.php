@extends('layouts.admin') {{-- Your main layout --}}
            ...
            <div class="card-body">
                ...
                <nav>
                    <div class="mb-3 nav nav-underline nav-tabs justify-content-between p-0 border-bottom rounded-0"
                        ...
                        <div class="d-flex align-items-center gap-3">
                            <button class="nav-link d-flex align-items-center rounded-0" id="nav-upload-files-tab"
                                data-bs-toggle="tab" data-bs-target="#nav-upload" type="button" role="tab"
                                aria-controls="nav-upload" aria-selected="false">Upload Media</button>
                ...
                <div class="tab-content iq-tab-fade-up" id="nav-tab-content">
                            ...
                            <div class="col-12 text-center mb-3">
                                <div class="input-group btn-file-upload">
                                    <button class="input-group-text form-control" type="button"
                                        onclick="document.getElementById('file_url_media').click()" style="height:16rem">
                    ...
                    <div class="tab-pane fade show active" id="nav-media" role="tabpanel"
                        ...
                        <div class="text-end">
                            <button class="btn btn-md btn-primary mt-2" id="mediaSubmitButton" type="button">Save</button>
                        </div>
                    </div>
                    {{-- <div class="tab-pane fade show active" id="nav-media" role="tabpanel"
                        aria-labelledby="nav-media-library-tab">
                        <div class="media-scroll pe-3">
                            <h6 id="no_data" class="text-center d-none">No media found.</h6>
                            <div class="d-flex gap-5 flex-wrap justify-content-center" id="media-container">
                                @foreach ($media as $item)
                                    <div class="iq-media-images position-relative" id="media-{{ $item->id }}">
                                        @if ($item->type == 'image')
                                            <img class="img-fluid object-fit-cover cursor-pointer"
                                                src="{{ asset('storage/' . $item->url) }}" style="width:10rem;height:10rem;"
                                                loading="lazy">
                                        @else
                                            <video class="img-fluid object-fit-cover" style="width:10rem;height:10rem;"
                                                controls>
                                                <source src="{{ asset('storage/' . $item->url) }}" type="video/mp4">
                                            </video>
                                        @endif
                                        <button
                                            class="btn btn-danger position-absolute top-0 start-0 m-2 py-1 px-2 iq-button-delete"
                                            onclick="deleteImage('{{ $item->url }}', {{ $item->id }})">
                                            <i class="ph ph-trash"></i>
                                        </button>
                                        <p class="media-title pt-2 mb-0" data-bs-toggle="tooltip"
                                            data-bs-title="{{ basename($item->url) }}">{{ basename($item->url) }}</p>
                                    </div>
                                @endforeach
                            </div>
                            <div id="loading-spinner" class="text-center mt-3"
...
@push('scripts')
    ...
    <script>
        function loadMedia(query = '', page = 1) {
            ...
            $.ajax({
                url: baseUrl + '/admin/getMediaStore',
                method: 'GET',
                data: {
                    query: query,
                    page: page
