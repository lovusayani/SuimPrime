...
@section('content')
    <div class="conatiner-fluid content-inner pb-0" id="page_layout">
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('admin.coupon.update', $coupon) }}" id="form-submit" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <!-- Coupon Code -->
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror"
                                    placeholder="Write description here..." rows="5">{{ old('description', $coupon->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="discount" id="discount"
                                            ...
                                            placeholder="e.g., 15.50" value="{{ old('discount', $coupon->discount) }}">
                                        @error('discount')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                        ...
                                        <select
                                            ...
                                            @foreach ($plans as $plan)
                                                <option value="{{ $plan->id }}"
                                                    {{ in_array($plan->id, old('subscription_plan_ids', $coupon->plans->pluck('id')->toArray())) ? 'selected' : '' }}>
                                                    {{ $plan->name }}</option>
                                            @endforeach
                                        </select>
