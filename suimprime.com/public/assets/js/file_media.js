...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (document.getElementById('submit-button')) {
        ...
        submitButton.addEventListener('click', function (event) {
            ...
            let remainingFiles = window.uploadedFiles.filter(file => !file.removed);
    ...
    /* if (document.getElementById('submit-button')) {
        ...
        submitButton.addEventListener('click', function (event) {
            ...
            if (remainingFiles.length > 0) {
                ...
                let xhr = new XMLHttpRequest();
                xhr.withCredentials = true; // must be added
            ...
            if (window.location.href === `${baseUrl}/admin/media-library`) {
 ...
 */
    ...
    function loadPaginatedImages() {
        ...
        fetch(`${baseUrl}/admin/get-media?page=${page}&perPage=${perPage}`)
    ...
    // When open modal with specific input
    $('#exampleModal').on('show.bs.modal', function (event) {
        let button = $(event.relatedTarget); // Button that triggered modal
        currentInputId = button.data('input');   // e.g. "iq-image-url"
        currentPreviewId = button.data('preview'); // e.g. "thumbnail-preview"
    });

    // When selecting media item
    $(document).on('click', '.media-item', function () {
        $('.media-item').removeClass('selected');
        $(this).addClass('selected');
        selectedMediaUrl = $(this).data('url');
    });

    // On Save button
    $('#saveMedia').on('click', function () {
        if (selectedMediaUrl && currentInputId) {
            let input = document.getElementById(currentInputId);
            let preview = document.getElementById(currentPreviewId);
...
function uploadChunk(file, index, start, end, chunkSize, uploadedFiles, progressBar) { // Added progressBar parameter
    ...
    xhr.open('POST', `${baseUrl}/admin/media-library/upload`, true);
    xhr.setRequestHeader('X-CSRF-TOKEN', document.querySelector('meta[name="csrf-token"]').getAttribute('content')); // Set CSRF token header
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadImages(query = '') {
        ...
        fetch(`${baseUrl}/admin/get-media?page=${page}&query=${encodeURIComponent(query)}`)
            .then(response => {
